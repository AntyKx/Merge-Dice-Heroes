...
import { getHeroProgress, HERO_XP_PER_VICTORY, heroXpRequirement } from "@/game/heroProgress";
...
function DungeonScreen() {
  ...
  return <section className="lobby-module-screen accent-violet skin-dungeon">
    ...
    <section className="dungeon-prep" aria-label="挑戰準備"><header className="dungeon-section-heading"><span>挑戰準備</span><button className={`dungeon-help ${showDungeonHelp ? "is-open" : ""}`} type="button" onClick={() => setShowDungeonHelp((open) => !open)} aria-expanded={showDungeonHelp} aria-controls="dungeon-help-panel" aria-label="查看試煉之門用途">?</button></header>{showDungeonHelp && <aside id="dungeon-help-panel" className="dungeon-help-panel" role="note"><button type="button" onClick={() => setShowDungeonHelp(false)} aria-label="關閉說明"><X size={13} /></button><b>試煉之門用途</b><p>在這裡挑戰帶有特殊規則的關卡，消耗對應體力並取得英雄與裝備養成素材。完成前一道試煉即可解鎖下一道門扉。</p></aside>}<div className="dungeon-energy"><img className="astervow-icon" src={ASTERVOW_UI_ICON_URLS.dungeonStamina} alt="" /><b>體力 {progress.stamina} / 20</b><span>每次挑戰消耗對應體力</span></div></section>
    <div className="astervow-divider" aria-hidden="true" />
...
function ResultOverlay() {
  ...
  return <div className="dialog-backdrop"><div className={`result-modal ${won ? "is-victory" : "is-defeat"} ${dungeon ? "trial-result-modal" : ""}`}><div className="result-seal">{won ? <Sparkles size={35} /> : <Shield size={35} />}</div><p>{won ? (dungeon ? "TRIAL CLEARED" : "THE CURTAIN RISES") : "THE CASTLE FELL"}</p><h3>{won ? (dungeon ? `${dungeon.title} 試煉通關！` : "勝利！") : "本局失利"}</h3><span>{run.message}</span>{won && dungeon && <section className="trial-result-rewards" aria-label="試煉通關獎勵"><header><Gift size={15} /><span>試煉結算</span><em>獎勵已入帳</em></header><div className="trial-result-reward-row"><img className="astervow-icon" src={ASTERVOW_UI_ICON_URLS.dungeonReward} alt="" /><span><small>命運碎晶</small><b>+{dungeon.reward.crystals}</b></span><small>{dungeon.reward.label}{dungeon.reward.equipmentId && progress.inventory.includes(dungeon.reward.equipmentId) ? " · 已收錄背包" : ""}</small></div><div className="trial-result-xp"><Sparkles size={16} /><span><small>隊伍經驗</small><b>每位英雄 +{HERO_XP_PER_VICTORY} EXP</b><em>{run.selectedHeroes.map((heroId) => HEROES[heroId].name).join(" · ")}</em></span></div></section>}<div className="result-stats"><div><b>{run.wave}</b><small>抵達波次</small></div><div><b>{run.activeTalents.length}</b><small>取得天賦</small></div><div><b>{run.combat.castleHp}</b><small>城堡生命</small></div></div>{dungeon ? <><button className="primary-cta wide-cta" onClick={() => openScreen("dungeon")}><Swords size={17} />查看試煉之門</button><button className="text-button" onClick={() => openScreen("title")}>回到王都</button></> : <><button className="primary-cta wide-cta" onClick={restartRun}><RotateCcw size={17} />再玩一局</button><button className="text-button" onClick={() => openScreen("title")}>回到首頁</button></>}</div></div>;
}
