...
function TitleScreen() {
  ...
  const notices: Partial<Record<LobbyModuleId, { label: string; reward?: boolean }>> = { equipment: hasUpgradeableEquipment ? { label: "可強化" } : undefined, shop: progress.shop.freeRefreshAvailable ? { label: "免費刷新" } : undefined, daily: claimableQuests ? { label: `可領取 ${claimableQuests}`, reward: true } : undefined, dungeon: hasDungeonAttempt ? { label: "可挑戰" } : undefined };
