...
function TitleScreen() {
  ...
  const landmark = (moduleId: LobbyModuleId, className: string) => { const module = LOBBY_MODULES[moduleId]; const notice = notices[moduleId]; const FacilityIcon = moduleId === "equipment" ? Hammer : moduleId === "shop" ? PackageOpen : moduleId === "daily" ? Check : Swords; return <button className={`island-landmark landmark-${className} island-stage-${islandStage} ${notice ? "has-notice" : ""}`} onClick={() => openScreen(moduleId)} aria-label={`${module.label}，${moduleStatus[moduleId]}，島嶼成長階段 ${islandStage + 1}`}><span className={`facility-building facility-${className}`} aria-hidden="true"><span className="facility-roof" /><span className="facility-wall"><FacilityIcon size={18} strokeWidth={2.8} /></span><span className="facility-door" /></span><span className="landmark-art"><i>{module.icon}</i></span><strong>{module.label}</strong><small>{moduleStatus[moduleId]}</small>{notice && <em className={notice.reward ? "is-reward" : ""}>{notice.reward && <Gift size={9} />}{notice.label}</em>}</button>; };
  ...
  return <section className={`lobby-screen island-lobby lobby-progress-${completedQuests} map-stage-${islandStage} ${departing ? "is-departing" : ""}`}>
    ...
    <section className="island-landmarks" aria-label="冒險大廳入口">
      ...
      <button className="island-landmark landmark-guide" onClick={() => openScreen("guide")}><span className="facility-building facility-guide" aria-hidden="true"><span className="facility-roof" /><span className="facility-wall"><BookOpen size={18} strokeWidth={2.8} /></span><span className="facility-door" /></span><span className="landmark-art"><BookOpen size={15} /></span><strong>策略圖鑑</strong><small>骰型與職業</small></button>
    </section>
